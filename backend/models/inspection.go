...
const (
	InspectionStatusOpen       = "待处理"
	InspectionStatusProcessing = "处理中"
	InspectionStatusResolved   = "已解决"
)
...
type Inspection struct {
	...
	AssigneeID      *uint             `json:"assignee_id" gorm:"index"` // 责任人用户ID
	Assignee        *User             `json:"assignee,omitempty" gorm:"foreignKey:AssigneeID"`
	AssigneeName    string            `json:"assignee_name" gorm:"size:100"`     // 责任人显示名
	...
	EscalationLevel int               `json:"escalation_level" gorm:"default:0"` // 升级次数
	LastRespondedAt *time.Time        `json:"last_responded_at"`                 // 最近响应时间
	LastEscalatedAt *time.Time        `json:"last_escalated_at"`                 // 最近升级时间
	Remark          string            `json:"remark"`                            // 备注
	Images          []InspectionImage `json:"images,omitempty" gorm:"foreignKey:InspectionID"`
	Events          []InspectionEvent `json:"events,omitempty" gorm:"foreignKey:InspectionID"`
...
type InspectionQuery struct {
	...
	Escalated  string `form:"escalated"`
